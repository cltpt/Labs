resource "azurerm_resource_group" "zerotrust_storageaccount7" {
  name     = "zerotrust_storageaccount_rg7"
  location = "East US"
}

resource "azurerm_storage_account" "stgacttlsidndadenrhtssaklen" {
  # You'll need to change the name of your storage account in the event that someone has already used this name
  name                     = "zsatlsidndadenrhtssaklen"
  resource_group_name      = azurerm_resource_group.zerotrust_storageaccount7.name
  location                 = azurerm_resource_group.zerotrust_storageaccount7.location
 ...
 # }
  ...
  depends_on = [azurerm_resource_group.zerotrust_storageaccount7]
}