resource "azurerm_resource_group" "zerotrust_storageaccount5" {
  name     = "zerotrust_storageaccount_rg5"
  location = "East US"
}

resource "azurerm_storage_account" "stgacttlsidndadenrhts" {
  # You'll need to change the name of your storage account in the event that someone has already used this name
  name                     = "ztstgaccttlsidndadenrhts"
  resource_group_name      = azurerm_resource_group.zerotrust_storageaccount5.name
  location                 = azurerm_resource_group.zerotrust_storageaccount5.location
  ...
  depends_on = [azurerm_resource_group.zerotrust_storageaccount5]
}